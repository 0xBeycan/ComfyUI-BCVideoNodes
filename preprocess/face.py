...
def face_bboxes_from_pose(pose_data, width, height, face_padding=0):
    """One (x1, y1, x2, y2) face box per frame of `pose_data`, from its face keypoints, grown
    by `face_padding` pixels on every side and kept inside the frame."""
...
def crop_faces(images, pose_data, face_padding=0, face_bboxes=None):
    ...
    cut as it is instead of the boxes the face keypoints give; `face_padding` applies only to
    the computed boxes. What face_bboxes overrides is named in one console line."""
    ...
    if face_bboxes is not None:
        ...
        log.info("face_bboxes connected: cut as given; pose_data's face keypoints"
                 + (f" and face_padding {face_padding}" if face_padding else "") + " not used")
    else:
        boxes = face_bboxes_from_pose(pose_data, W, H, face_padding)
        if len(boxes) != B:
