...
from .temporal import (BOX_WINDOW, DROPPED, MAX_GAP, MAX_RESIDUAL, MAX_STEP, MEASURED, RECOVERED, REPLACED,
                       boxes_over_time, keypoints_over_time)
...
class PoseConfig:
    ...
    temporal: bool = field(default=True, metadata={
        "doc": "Read boxes and keypoints against the frames around them (fill short gaps, replace glitches); off ignores the temporal_* fields (not SAM 3 Config's temporal)"})
...
def _to_device(*models):
    """Bring the models to the compute device together, freeing VRAM held by other models if
    needed; ComfyUI moves them back out when another model needs the room."""
...
def detect(detector, pose_model, images, bboxes=None, config=None):
    ...
    images_np = images.numpy() if isinstance(images, torch.Tensor) else np.asarray(images)
    ...
    return pose_data, [tuple(float(v) for v in box[:4]) for box in boxes]


def draw(pose_data, body_stick_width=-1, hand_stick_width=-1, draw_head=True, draw_threshold=0.5):
...
def key_frame_body_points(pose_data, threshold=0.5):
    """KEY_FRAME's body keypoints in KEY_FRAME_BODY_POINTS that reach `threshold`, as the JSON
    string KJNodes PointsEditor emits and easy-sam3 reads as `positive_coords`:
    '[{"x": 50, "y": 120}, ...]' in frame pixels. '[]' when none does."""
    ...
    points = (confident[:, :2] * np.array([[meta["width"], meta["height"]]])).astype(np.int32)
