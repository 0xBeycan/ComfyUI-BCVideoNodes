...
from .blocks import PAFPN, Conv, ConvChain, SPPBottleneck, parameter
...
class C2f(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.blocks = nn.ModuleList(ConvChain(c) for c in cfg["blocks"])
        self.cv2 = Conv(cfg["cv2"])

    def forward(self, x):
        y = list(self.cv1(x).split(self.split, dim=1))
...
class PSA(nn.Module):
    ...
    def forward(self, x):
        a, b = self.cv1(x).split(self.split, dim=1)
        b = b + self.attn(b)
        b = b + self.ffn(b)
...
class Detect(nn.Module):
    ...
    def __init__(self, cfg):
        super().__init__()
        self.box = nn.ModuleList(ConvChain(c) for c in cfg["box"])
        self.cls = nn.ModuleList(ConvChain(c) for c in cfg["cls"])
