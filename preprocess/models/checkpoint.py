...
from .rtmw import RTMWNet
from .vitpose import ViTPoseNet
from .yolo import YOLOv10Net
...
def build(architecture, config, device="meta"):
    ...
    if architecture not in ARCHITECTURES:
        raise ValueError(f"unknown model architecture {architecture!r}; expected one of {sorted(ARCHITECTURES)}")
    with torch.device(device):
        return ARCHITECTURES[architecture](config).eval()
...
def save(net, architecture, path, dtype, extra=None):
    ...
    if type(net) is not ARCHITECTURES[architecture]:
        raise TypeError(f"{type(net).__name__} is not the {architecture} module ({ARCHITECTURES[architecture].__name__})")
