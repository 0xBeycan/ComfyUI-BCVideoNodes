...
"""RTMW-l's raw output, SimCC column and row classifications, to keypoints in frame coordinates,
[N, K, 3] rows of x, y and a 0..1-ish confidence. Kept apart from the wrappers so the offline
conversion can decode exactly the way the nodes do without importing ComfyUI."""
import numpy as np
...
def decode_simcc(simcc_x, simcc_y, center, scale, input_size, conf_scale=SIMCC_CONF_SCALE):
    ...
    from ...libs.pose_utils.pose2d_utils import transform_preds
