...
"""YOLOv10x as the pose pipeline calls it: the person box, in frame coordinates, on every frame."""
import numpy as np
import torch

from ...libs.bbox import whole_frame_box
...
class Yolo(NativeModel):
    ...
    def postprocess(self, output, shape_raw, cat_id=[1]):
        ...
        from ...libs.pose_utils.pose2d_utils import box_convert_simple
    ...
    def forward(self, img, shape_raw, **kwargs):
        ...
        person_results = [[{'bbox': whole_frame_box(1.*shape_raw[i][1], 1.*shape_raw[i][0]), 'track_id': -1}] for i in range(len(outputs))]
